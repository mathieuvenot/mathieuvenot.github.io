...
// Javascript for stacks_in_522_page24
// ---------------------------------------------------------------------

// Each stack has its own object with its own namespace.  The name of
// that object is the same as the stack's id.
stacks.stacks_in_522_page24 = {};

// A closure is defined and assigned to the stack's object.  The object
// is also passed in as 'stack' which gives you a shorthand for referring
// to this object from elsewhere.
stacks.stacks_in_522_page24 = (function(stack) {
...
$(function() {
	...
	$('#stacks_in_522_page24').find('li').unwrap().unwrap();

	// Fixed width importing

	if($('#stacks_in_522_page24-sf-import').length){
		var ec1 = $('#extraContainer1');
		if(ec1.length){
			$('#stacks_in_522_page24').appendTo(ec1);
		}
	}

	var dynSrc = $('#stacks_in_522_page24 .slides li').first().find('img').attr('src');

	firstSFimg = $("<img />").attr("src",dynSrc+ "?" + new Date().getTime());

	$(firstSFimg).load(function() {

		var thisSF = $('#stacks_in_522_page24');
			...
			if($('#stacks_in_522_page24-import').length){
				if($('meta[name=viewport]').length){
					$('#extraContainer1:has(#stacks_in_522_page24)').parent().find('#extraContainer1 ~ img').remove();
				}else{
